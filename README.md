...
Today it works and it's hard to detect and today it's [DATA].
...
- `random`: A UUID which is unique for every logging session.
...
Checking on [VirusTotal](LINK), I found no antivirus able to detect this trojan.
...
- Visit [LINK ALLA PAGINA DEL TEST] and downvote it (not really sure this one helps).
